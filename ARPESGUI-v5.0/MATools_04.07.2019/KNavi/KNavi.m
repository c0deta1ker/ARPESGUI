...
% Last Modified by GUIDE v2.5 29-Dec-2017 14:38:53
...
surfNormX = SurfNormX(hv,eB,kx,thtM,thtA);
% display results
set(handles.textOrgSurfNormX,'String',['SurfN//x = ' num2str(surfNormX)])
set([handles.editKxSurfNormX handles.editKySurfNormX handles.editKzSurfNormX], 'String',num2str(surfNormX))
...
global ePhi alpha eB
% read in the parameters
surfNormX=str2num(get(handles.editKxSurfNormX,'String'));
hv=str2num(get(handles.editKxHv,'String')); if hv<ePhi; hv=[]; end
kx=str2num(get(handles.editKxKx,'String')); 
thtM=str2num(get(handles.editKxThtM,'String')); thtMPhys=thtM+surfNormX;
thtA=str2num(get(handles.editKxThtA,'String'));
% calculation of kx/thtM/thtA
switch get(handles.popupmenuKx,'Value')
   case 1
      if isempty(surfNormX*hv*thtM*thtA); msgbox('Some Inputs or Parameters Empty','modal'); return; end
      kx=0.5124*sqrt(hv-ePhi+eB)*sind(thtMPhys+thtA)-2*pi*hv*cosd(alpha+thtMPhys)/12400;
      set(handles.editKxKx,'String',num2str(kx))
   case 2
      if isempty(surfNormX*hv*kx*thtA); msgbox('Some Inputs or Parameters Empty','modal'); return; end 
      [thtM,~,exitFlag]=fsolve(@(x) 0.5124*sqrt(hv-ePhi+eB)*sind(x+surfNormX+thtA)-...
         2*pi*hv*cosd(alpha+x+surfNormX)/12400-kx,0,optimset('Display','off','TolFun',1e-12));
      if exitFlag~=1; thtM=[]; msgbox('Error: Check Input Parameters','modal'); end
      set(handles.editKxThtM,'String',num2str(thtM))
   case 3
      if isempty(surfNormX*hv*kx*thtM); msgbox('Some Inputs or Parameters Empty','modal'); return; end 
...
% read in the parameters
thtA=str2num(get(handles.editKxThtA,'String'));
surfNormX=str2num(get(handles.editKySurfNormX,'String'));
thtM=str2num(get(handles.editKyThtM,'String')); thtMPhys=thtM+surfNormX;
surfNormY=str2num(get(handles.editKySurfNormY,'String'));
hv=str2num(get(handles.editKyHv,'String')); if hv<ePhi; hv=[]; end
ky=str2num(get(handles.editKyKy,'String'));
tlt=str2num(get(handles.editKyTlt,'String')); tlt=tlt-surfNormY;
% calculation of ky/TLT
switch get(handles.popupmenuKy,'Value')
   case 1
      if isempty(thtM*surfNormX*hv*tlt); msgbox('Some Inputs or Parameters Empty','modal'); return; end 
   ...
   case 2
      if isempty(thtM*surfNormX*hv*ky); msgbox('Some Inputs or Parameters Empty','modal'); return; end  
      ...
      set(handles.editKyTlt,'String',num2str(tlt+surfNormY))
end
...
global ePhi alpha eB
% inputs
surfNormX=str2num(get(handles.editKzSurfNormX,'String'));
thtM=str2num(get(handles.editKzThtM,'String')); thtMPhys=thtM+surfNormX;
...
switch get(handles.popupmenuKz,'Value')
   case 1
      if isempty(thtM*surfNormX*kx*ky*v000*hv); msgbox('Some Inputs or Parameters Empty','modal'); return; end  
   ...
   case 2
      if isempty(thtM*surfNormX*kx*ky*v000*kz); msgbox('Some Inputs or Parameters Empty','modal'); return; end 
      if get(handles.popupmenuKzUnits,'Value')==2; kz=kz*tpc; end   % kz units        
    ...
    case 3
      if isempty(thtM*surfNormX*kx*ky*hv*kz); msgbox('Some Inputs or Parameters Empty','modal'); return; end 
      if get(handles.popupmenuKzUnits,'Value')==2; kz=kz*tpc; end   % kz units 
